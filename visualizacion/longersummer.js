        ...
        function updateVisualization(yearIndex, animate = true) {
            ...
            if (animate) {
                gsap.timeline()
                    ...
                    .to(yearInfoContent, { duration: 0.2, opacity: 1 })
                    .to(dayMarkers, { backgroundColor: '#e0e0e0', duration: 0.3, stagger: 0.001 }, 0)
                    .to(dayMarkers.slice(startDay, endDay + 1), { backgroundColor: '#c90022', duration: 0.3, stagger: 0.001 }, ">.1");
            } else {
                yearInfoContent.innerHTML = `${yearHTML}${detailsHTML}`;
                dayMarkers.forEach((marker, i) => {
                    marker.style.backgroundColor = (i >= startDay && i <= endDay) ? '#c90022' : '#e0e0e0';
                });
            }
        }
